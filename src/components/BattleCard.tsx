...
const BattleCard = ({ battle, index, onClick }: BattleCardProps) => {
  const resultStyle = battle.result === "victory"
    ...
    >
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="relative lg:w-2/5 h-80 lg:h-auto overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-l from-black/70 via-black/30 to-transparent lg:bg-gradient-to-r" />
          
          {/* Year Badge */}
          <div className="absolute top-4 left-4 px-4 py-2 rounded-xl glass-section">
            <span className="text-foreground font-bold text-xl">{battle.year}</span>
          </div>

          {/* Result Badge */}
          <div className={`absolute bottom-4 left-4 flex items-center gap-2 px-3 py-1 rounded-full backdrop-blur-md border ${resultStyle}`}>
